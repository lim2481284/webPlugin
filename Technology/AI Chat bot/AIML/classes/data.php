...
class data {
	...
	public function ProcessInput() {
		...
		$this->output = "I do not understand that command, but I will learn from it, or you can get your answer from here  <br> <br> <center><a target='_blank' class='url' href='https://www.google.com/search?q=$keyword'> $keyword. </a> </center>";
	...
	*/
	...
	public function ShowHistoryChat() {
		$db = $GLOBALS['database'];
		$c = "";
		$sql = "SELECT chat_id as id, client_message as input, server_response as response
						FROM chat
						WHERE chat_id <> ''
						ORDER BY datetime_response desc
						";
		$result = $db->Query($sql);
		if ($db->NumRows($result) == 0) {
